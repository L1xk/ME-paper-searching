...
description: Build, configure, test, audit, or run a weekly English-literature radar for microbial metabolic engineering, enzyme engineering, and wet-lab-validated AI for Protein. Use when the user asks for ME × Protein paper discovery, score-ranked Top and non-Top screening, strict preprint exclusion, Chinese summaries, review quotas, QQ email delivery, DeepSeek cost control, GitHub Actions scheduling, deduplication history, or this radar repository.
---
...
5. Use DeepSeek `deepseek-v4-flash` in two stages: first run compact semantic/evidence screening for the candidate pool, then generate Chinese titles, reasons, and summaries only for the final selected papers, capped at 15. Use non-thinking JSON mode, validate every response, and retry malformed or empty JSON. Isolate a single candidate after its retries are exhausted, record the failure, and abort only after the configured number of consecutive screening failures indicates a service outage.
...
9. Target 10–15 formal papers and cap the issue at 15, including at least 2 and at most 3 qualified reviews. If fewer than 10 papers pass every quality gate, deliver the actual qualified count instead of failing on total count. Recent means publication within 30 days of the issue date; allow at most 8 historical papers from the rolling six-year window. Do not relax the review minimum or relabel an article as a review; if the dedicated review lane is exhausted, stop and alert. When the qualified pool permits, keep each canonical journal to at most 2 papers and each research track to at most 4; relax only these diversity limits when required to satisfy the review quota or fill higher-scoring slots, and record the relaxation.
10. Allow both Top and non-Top articles and reviews to compete by final score after the same scope, evidence, document-type, link, and score gates. Require final_score at least 72 for all papers. Do not impose a Top-journal minimum or a non-Top count cap. Preserve the configured +12 Top-journal score bonus, use Top status only as a tie-break after final score, label qualified non-Top papers in the email, and keep exceptional-novelty evidence as optional context rather than an admission requirement.
