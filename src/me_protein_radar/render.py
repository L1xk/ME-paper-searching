...
def render(selection: dict[str, Any], warnings: list[str] | None = None) -> str:
    ...
    stats_html = ""
    if stats:
        stats_html = f'<div style="margin-top:8px">候选池：正式 {int(stats.get("formal", 0))} · Top 期刊 {int(stats.get("top_journal", 0))} · 预印本推送已关闭</div>'
...
<tr><td style="padding:20px 22px 28px;font-size:12px;line-height:1.6;color:#6b7681">摘要级核验仅复述公开摘要明确支持的结论；开放全文仅使用合法公开来源；预印本不进入候选和推送。{stats_html}{warning_html}</td></tr></table></td></tr></table></body></html>'''
