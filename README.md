...
1. Crossref、PubMed、Europe PMC 和 OpenAlex 按来源并行检索正式发表论文；除主题检索外，PubMed/Europe PMC 使用分组期刊条件，Crossref 对重点化学与工程期刊逐刊定向召回。
...
5. 通过 `config/journals.json` 将期刊全名和简称归一化，区分直接 Top、条件 Top 与仅综述 Top；再按研究方向、证据等级、论文类型、时间池和弹性多样性选稿。创新例外会在邮件中单独标注并展示证据依据。
